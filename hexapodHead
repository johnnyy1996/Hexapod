...
//this function was designed to work with the Arduino software but was unable to work the way it was meant to due to the fact that Arduino does not support the fstream library.
void hexapodHead::cycleMotorOutput(char * motor_output_file_left, char * motor_output_file_right){
    string headerLeft, headerRight;
    float outputLeft, outputRight;
    int calculatedLeftPosition, calculatedRightPosition;
    ifstream myMotorOutputFileLeft;
    ifstream myMotorOutputFileRight;
    
    myMotorOutputFile.open(motor_output_file_left);
    myMotorOutputFile.open(motor_output_file_right);
    
    if (!motor_output_file_left)
        cout << "Left file failed to open" << endl;
    if (!motor_output_file_right)
        cout << "Right file failed to open" << endl;
    
    myMotorOutputFileLeft >> headerLeft;
    myMotorOutputFileRight >> headerRight;
    
    while (!myMotorOutputFileLeft.eof() && !myMotorOutputFileRight.eof()) {
        myMotorOutputFileLeft >> outputLeft;
        calculatedLeftPosition = outputLeft * 1220;
        myMotorOutputFileRight >> outputRight;
        calculatedRightPosition = outputRight * 1280;
        setUpMandibles(calculatedLeftPosition, calculatedRightPosition);
        delay(1500);
    }
    
    myMotorOutputFileLeft.close();
    myMotorOutputFileRight.close();
}
